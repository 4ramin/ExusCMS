	...
	class dir{
		...
		public static function make($args){
			...
			//make array directory
				foreach($path as $val){
					if(!is_dir($val)){
						if(!mkdir($val, $mode, true)) return FALSE;
					}
				}
			}
			return TRUE;
		...
		public static function _list(\stdClass $args){
			...
			//args
			$path = $args->path;
			if(isset($args->target)) $list_target = $args->target;
			$include_subdirectory = $args->show_sub;
			if(!isset($include_subdirectory)) $include_subdirectory = FALSE;
			$include_path = $args->show_path;
			if(!isset($include_path)) $include_path = FALSE;
