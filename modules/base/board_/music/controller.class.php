	...
	class board_controller extends controller_abstract implements controllerInterface 
	{
		...
		function getAudiolyrics() 
		{
			...
			if ($lyrics) 
			{
				...
				preg_match_all('|(?:<strLyric>)([[\/]?[\d]{2}:[\d]{2}.[\d]{2}\].*)(?:<br>)(?:</strLyric>)|U', $lyrics, $output);
				...
				{
					// Convert time
					$time_date = preg_replace("/[[\/]?([\d]{2}):([\d]{2}).([\d]{2})\](.*)<br>/U",'<div ms="$3" timestamp="$1:$2">$4</div>', $output[1][0].'<br>');
					
					// Convert Seconds
					$time_date = preg_replace_callback('/"([\d]{2}):([\d]{2})"/', function($m) 
					{
						return date($m[1] * 60 + $m[2]);
					}, $time_date);
					
					// Convert Minutes
					$time_date = preg_replace_callback('/"([\d]{2})"/', function($m) 
					{
						return (int)($m[1]);
					}, $time_date);
					
					return $this->base->response("type", "success", "html", $time_date);
				}
				else 
				{
