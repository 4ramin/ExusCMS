<?php
	...
	class board_model extends board 
	{
		...
		function getID3vContext($target_file) 
		{
			if (!$this->getID3) 
			{
				include_once(__DIR."/library/getid3/getid3.php");
	...
	/* Tag, ETC */
		 ...
		 * @param int $pgx
		 */
		function getCategoryListWithoutChildren($module) 
		{
			return  db::Query('SELECT','def_category',[
		 ...
		 * @param int $pgx
		 */
		function getCategoryChildren($module, $subsrl) 
		{
			return  db::Query('SELECT','def_category',[
				['AND', 'module', '=', ':args1', $module],
	...
	/* Module */
		 ...
		 * @param string $module
		 */
		function getModuleLayoutByBoardName($module) 
		{
			return db::Query('SELECT','def_module',[
				['', 'bdname', '=', ':args1', $module]
			],'layout', 'one');
	...
	/* File */
		 ...
		 * @param array $array
		 */
		function getFilesInTarget($array) 
		{
		...
		function getPopularDocumentListCount($module, $down_count) 
		{
			return db::Query('SELECT','def_file',[
				['AND', 'module', '=', ':args1', $module],
	...
	/* Document List */
		 ...
		 * @param int $list_count
		 */
		function getPopularDocumentListBySortIndex($module, $down_count, $page_start, $list_count) 
		{
		 ...
		 * @param   str  $article
		 */
		function getDocumentListByOrder($module, $page_start, $list_count, $article) 
		{
			return db::Query('SELECT','def_document_music',[
				['', 'module', '=', ':args1', $module],
				['ORDER', $article, 'desc'],
		...
		function getDocumentListLIMIT($module, $page_start, $board_count) 
		{
			...
			],'def_document_music.*, (SELECT count(b.srl) FROM def_file b WHERE b.target = def_document_music.file_sequence) as `file_count`, (SELECT count(c.comment_srl) FROM def_comment c WHERE c.document_srl = def_document_music.srl_bd) as `comment_count`, (SELECT name FROM def_category d WHERE d.category_srl = def_document_music.category_srl) as `category_caption`', 'all'));
		}
