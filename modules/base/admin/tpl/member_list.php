...
<div class="list_area"><?php include('_header.php'); ?>
	<div class="admin_content list_content content newclearfix">
		...
		<table id="documentListTable" class="x_table x_table-striped x_table-hover">
			<thead>
				<tr>
					<th scope="col" class="title">닉네임</th>
					<th scope="col" class="nowr">아이디</th>
					<th scope="col" class="nowr">관리자</th>
			...
			<tbody>
				...
				<tr>
					<td>
						<?php echo $val['nick_name']; ?>
					</td>
					<td>
						<?php echo $val['user_id']; ?>
					</td>
					<td>
						<?php echo $val['is_admin']===1 ? 'Y' : 'N' ; ?>
					</td>
					<td>
		...
		<form method="get" onsubmit="return procFilter(this, search)" class="bd_pg clear">
			...
			<span class="btn_img itx_wrp" method="get" no-error-return-url="true">
				<button type="submit" onclick="jQuery(this).parents('form.bd_srch_btm').submit();return false;" class="ico_16px search"><?php echo $this->admin->lang['search']; ?></button>
			...
			<span class="select">
				<select name="type">
					<option <?php echo $_GET['type']=='nickname' ? 'selected="selected"' : ''?> value="nickname"><?php echo $this->admin->lang['nickname']; ?></option>
					<option <?php echo $_GET['type']=='tag' ? 'selected="selected"' : ''?> value="tag"><?php echo $this->admin->lang['tag']; ?></option>
					<option <?php echo $_GET['type']=='author' ? 'selected="selected"' : ''?> value="author"><?php echo $this->admin->lang['singer']; ?></option>
				</select>
			</span>
		</form>
